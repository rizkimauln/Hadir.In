...
for ($i = 1; $i <= $jumlahhari; $i++) {
    ...
    $status = "-";
    // Cek hari libur atau Minggu dulu
    $hari = Carbon::parse($tanggal)->translatedFormat('l');
    $isMinggu = $hari === 'Minggu';
    $isLibur = isset($liburMerahSet[$tanggal]) || isset($cutiBersamaSet[$tanggal]);

    // Jika hari libur atau Minggu, status kosong
    if ($isMinggu || $isLibur) {
        $status = "Libur/Hari Minggu";
    } else {
        if ($presensiHarian) {
            $status = $presensiHarian->jam_in > '08:00:00'
                ? "Terlambat " . selisih('08:00:00', $presensiHarian->jam_in)
                : "Tepat Waktu";
        } elseif ($izinHarian) {
...
$remaining = collect($rekapData)->slice(16);
...
<section class="sheet padding-10mm">
    ...
    <table class="tabeldatakaryawan">
        ...
        <tr><td style="padding-left: 10px">NIK</td><td>:</td><td>{{ $karyawan->nik }}</td></tr>
        <tr><td style="padding-left: 10px">Nama Karyawan</td><td>:</td><td>{{ $karyawan->nama_lengkap }}</td></tr>
        <tr><td style="padding-left: 10px">Jabatan</td><td>:</td><td>{{ $karyawan->jabatan }}</td></tr>
        <tr><td style="padding-left: 10px">Departemen</td><td>:</td><td>{{ $karyawan->nama_dept }}</td></tr>
        <tr><td style="padding-left: 10px">No HP</td><td>:</td><td>{{ $karyawan->no_hp }}</td></tr>
    </table>
    @endif
    ...
    <table class="tabelpresensi">
        ...
        @foreach($chunk as $d)
        <tr style="{{ $d['highlight'] ? 'background-color: #f8d7da;' : '' }}">
            <td>{{ $d['no'] }}</td>
